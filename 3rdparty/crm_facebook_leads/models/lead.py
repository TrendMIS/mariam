...
class CrmFacebookPage(models.Model):
    ...
    def get_forms(self):
        r = requests.get("https://graph.facebook.com/v2.12/" + self.name + "/leadgen_forms",
                         params={'access_token': self.access_token}).json()
        if r.get("error"):
            raise AccessDenied(r["error"]['message'])
...
class CrmFacebookForm(models.Model):
    ...
    def get_fields(self):
        ...
        r = requests.get("https://graph.facebook.com/v2.12/" + self.facebook_form_id,
...
class CrmLead(models.Model):
    ...
    def get_adset(self, lead):
        ...
        if not lead.get('adset_id'):
            return ad_obj
        if not ad_obj.search(
                [('facebook_adset_id', '=', str(lead['adset_id']))]):
            return ad_obj.create({
                'facebook_adset_id': lead['adset_id'], 'name': lead['adset_name'], }).id

        return ad_obj.search(
            [('facebook_adset_id', '=', str(lead['adset_id']))], limit=1)[0].id
    ...
    def prepare_lead_creation(self, lead, form):
        ...
        partner_vals = {
            'mobile': vals['mobile'] if 'mobile' in vals.keys() else False,
            'email': vals['email_from'] if 'email_from' in vals.keys() else False,
            'phone': vals['phone'] if 'phone' in vals.keys() else False,
            'name': vals['contact_name'] if 'contact_name' in vals.keys() else False,
            'customer': True
        }
        created_partner = False
        if not vals.get('email_from') and lead.get('email'):
            vals['email_from'] = lead['email']
            partner_vals['email'] = lead['email']
        if not vals.get('contact_name') and lead.get('full_name'):
            vals['contact_name'] = lead['full_name']
            if not partner_vals['name']:
                partner_vals['name'] = lead['full_name']
        if not vals.get('phone') and lead.get('phone_number'):
            vals['phone'] = lead['phone_number']
            partner_vals['phone'] = lead['phone_number']
        vals.update({
            ...
            'medium_id': form.medium_id and form.medium_id.id or
                         self.get_ad(lead),
            'user_id': SUPERUSER_ID,  # form.team_id and form.team_id.user_id and form.team_id.user_id.id or False,
            'facebook_adset_id': self.get_adset(lead),
            'facebook_form_id': form.id,
            'facebook_date_create': lead['created_time'].split('+')[0].replace('T', ' ')
        })
        if partner_vals['mobile']:
            self.env.cr.execute(
                "SELECT id FROM res_partner WHERE mobile is not null AND RIGHT(mobile, 10)=%s",
                (partner_vals['mobile'][-10:],))
            partner_data = self.env.cr.fetchone()
            if partner_data:
                partner_object = self.env['res.partner'].browse(partner_data[0])
                vals.update({'partner_id': partner_object.id})
                # get team leader of partner sales person
                if partner_object.user_id:
                    user_team_object = self.env['crm.team']._get_default_team_id(
                        user_id=partner_object.user_id.id)
                    if user_team_object.user_id:
                        vals.update({'user_id': user_team_object.user_id.id})

        if not vals.get('partner_id') or vals['partner_id'] == False:
            created_partner = self.env['res.partner'].create(partner_vals)
            if created_partner:
                vals.update({'partner_id': created_partner.id})
        return vals
    ...
    def get_opportunity_name(self, vals, lead, form):
        if not vals.get('name'):
            vals.update({'name': form.project_id.id})
        return vals['name']
