...
class normal_payments(models.Model):
    ...
    def action_confirm(self):
        ...
        if self.send_rec_money == 'send':
            ...
            credit_account = [{'account': self.payment_method.payment_debit_account_id.id, 'percentage': 100}]
        else:
            credit_account = [{'account': self.account_id.id, 'percentage': 100, 'analyitc_id': self.analyitc_id.id, }]
            debit_account = [{'account': self.payment_method.payment_debit_account_id.id, 'percentage': 100}]
        ...
        if self.payment_subtype:
            for check in self.pay_check_ids:
                ...
                check_line_val['notespayable_id'] = self.payment_method.payment_debit_account_id.id
                check_line_val['notes_rece_id'] = self.payment_method.payment_debit_account_id.id
